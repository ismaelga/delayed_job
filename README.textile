...
This repo has only two lines different from the official DelayedJob repo so no big deal.

Intend was to make <pre>rake jobs:work</pre> command to stop when there is no more jobs to run. This is because we wanted to run it in a cron job and well, there should be a way to end the worker and this was maybe the easier way.

h2. How to use this verson?

Just place it in the Gemfile before any delayed_job specific gem like delayed_job_active_record.

Here is an exemple
<pre>
gem 'delayed_job', :git => 'git://github.com/ismaelga/delayed_job.git'
